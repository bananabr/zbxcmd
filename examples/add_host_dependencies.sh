...
PARENT_TRIGGERS=`python zbxcmd.py -s $ZABBIX_BASE_URL -u $ZABBIX_USER -p $ZABBIX_PASSWORD -c host --method=get -F "host:$PARENT_HOST" -f hostid -f host -F "priority:2" --include-triggers | jq ".triggers[] | select((.priority | tonumber) > $TRIGGER_PRIO) | .triggerid" | sed 's/"//g'`
echo $PARENT_HOST_ID
